...
export class WalletListComponent implements OnInit {
  ...
  DeleteWallet(w_id: number): void {
    this.showAlert(
      'Are You Sure, You want to Delete this Data?',
    ).then((result: SweetAlertResult<any>) => {
      if (result.isConfirmed) {
        this.apiService.AdminWalletDelete(w_id).subscribe(
          (data: any) => {
  ...
  EditWallet(w_id: Number): void {
    const modalRef = this.modalService.open(WalletAddComponent, { size: 'lg', backdrop: 'static', keyboard: false });
    modalRef.componentInstance.editid = w_id;
  ...
  TableDataList(){
    this.apiService.AdminGetWallet(this.w_id).subscribe(data => {
      debugger;
      if(data.responseCode == 200){
