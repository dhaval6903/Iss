...
import { OfficenotesAddComponent } from '../officenotes-add/officenotes-add.component';
...
@Component({
  selector: 'app-officenotes-list',
  templateUrl: './officenotes-list.component.html',
  styleUrl: './officenotes-list.component.css'
})
export class OfficenotesListComponent implements OnInit {
  ...
  DeleteOfficeNotes(n_id: number): void {
    this.showAlert(
      'Are You Sure, You want to Delete this Data?',
    ).then((result: SweetAlertResult<any>) => {
      if (result.isConfirmed) {
        this.apiService.AdminNotesDelete(n_id).subscribe(
          (data: any) => {
  ...
  EditOfficeNotes(n_id: Number): void {
    const modalRef = this.modalService.open(OfficenotesAddComponent, { size: 'lg', backdrop: 'static', keyboard: false });
    modalRef.componentInstance.editid = n_id;
  ...
  TableDataList(){
    this.apiService.AdminGetNotes(this.n_id).subscribe(data => {
      debugger;
      if(data.responseCode == 200){
