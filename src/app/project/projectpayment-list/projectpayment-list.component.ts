...
import { ProjectpaymentAddComponent } from '../projectpayment-add/projectpayment-add.component';
...
@Component({
  selector: 'app-projectpayment-list',
  templateUrl: './projectpayment-list.component.html',
  styleUrl: './projectpayment-list.component.css'
})
export class ProjectpaymentListComponent implements OnInit {
  ...
  DeleteProjectPayment(pp_id: number): void {
    this.showAlert(
      'Are You Sure, You want to Delete this Data?',
    ).then((result: SweetAlertResult<any>) => {
      if (result.isConfirmed) {
        this.apiService.AdminProjectPaymentDelete(pp_id).subscribe(
          (data: any) => {
  ...
  openEditProjectForm(pp_id: Number): void {
    const modalRef = this.modalService.open(ProjectpaymentAddComponent, { size: 'lg', backdrop: 'static', keyboard: false });
    modalRef.componentInstance.editid = pp_id;
  ...
  TableDataList(){
    this.apiService.AdminGetProjectPayment(this.pp_id).subscribe(data => {
      debugger;
      if(data.responseCode == 200){
