...
export class ClientListComponent implements OnInit {
  ...
  ClientDelete(cl_id: number): void {
    this.showAlert(
      'Are You Sure, You want to Delete this Data?',
    ).then((result: SweetAlertResult<any>) => {
      if (result.isConfirmed) {
        this.apiService.AdminClientDelete(cl_id).subscribe(
          (data: any) => {
  ...
  EditClient(cl_id: Number): void {
    const modalRef = this.modalService.open(ClientAddComponent, { size: 'lg', backdrop: 'static', keyboard: false });
    modalRef.componentInstance.editid = cl_id;
  ...
  TableDataList(){
    this.apiService.AdminGetClient(this.cl_id).subscribe(data => {
      debugger;
      if(data.responseCode == 200){
